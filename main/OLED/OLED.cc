...
/* 互斥保护，防止多个任务同时操作 GPIO bit-bang I2C 导致总线冲突 */
static SemaphoreHandle_t oled_i2c_mutex = NULL;

/* 初始化互斥锁 */
void OLED_InitMutex(void)
{
...
{
	if (oled_i2c_mutex) xSemaphoreTake(oled_i2c_mutex, portMAX_DELAY);
...
void OLED_I2C_Stop(void)
{
	...
	if (oled_i2c_mutex) xSemaphoreGive(oled_i2c_mutex);
}
...
void OLED_I2C_SendByte(uint8_t Byte)
{
	...
	/* ACK 检测：释放 SDA 并把 SDA 切到输入读取 ACK 电平 */
	OLED_W_SDA(1);
	gpio_set_direction(GPIO_SDA, GPIO_MODE_INPUT);
	OLED_W_SCL(1);
	esp_rom_delay_us(5);
	int ack = gpio_get_level(GPIO_SDA); // 0 表示 ACK
	OLED_W_SCL(0);
	gpio_set_direction(GPIO_SDA, GPIO_MODE_OUTPUT_OD);
	(void)ack; // 目前忽略 ACK，若为 NACK 可实现重试或错误处理
}
